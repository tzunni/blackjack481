...
import sys
from ai_algorithm_3 import AI as AI_Minimax
from ai_algorithm_2 import AI as AI_Second
...
text_Score = pygame.font.SysFont(fontType, 25)
...
def startGame():
    ...
    add_text("PRESS SPACE TO CONTINUE", text_SubHeading, screen, halfWidth, halfHeight + 80, white)
    pygame.display.update()
    beginning = True
    while beginning:
        for event in pygame.event.get():
...
def getPlayerNames():
    ...
    players.append(Player("AI Agent V3"))
    players.append(Player("AI Agent V2"))

    # Assign AI agents to specific players
    players[1].ai_agent = ai_minimax
    players[2].ai_agent = ai_second
...
def playTurns():
    ...
    pygame.init()
    screen = pygame.display.set_mode((screenWidth, screenHeight))
    pygame.display.set_caption(f"Play Round {round}")
    ...
    while turn < len(players):
        ...
        if currentPlayer.blackjack:
            # Skip the turn for players with blackjack
            turn += 1
            continue

        # Set current player's turn status
        for player in players:
            player.currentTurn = (player == currentPlayer)
            ...
            while keepHitting:
                ...
                if choice == 1:  # Hit
                    ...
                    if currentPlayer.count == 21:
                        print(f"{currentPlayer.name} got a blackjack!")
                        currentPlayer.blackjack = True
                        keepHitting = False
                    elif currentPlayer.count > 21:
                        print(f"{currentPlayer.name} busted.")
        ...
        # AI Turn Logic
            ...
            while keepHitting:
                ...
                ai_agent = getattr(currentPlayer, 'ai_agent', None)
                if ai_agent:
                    remaining_cards = dealer.deck.cards.copy()
                    dealer_visible_card = dealer.hand[0] if dealer.hand else None
                    action = ai_agent.decide_action(currentPlayer.hand, [dealer_visible_card], remaining_cards)
...
def drawTurn(surface, empty):
    ...
    for player in players:
        ...
        add_text(score_text, text_Score, surface, startX + 990, startY + 690, white)
        startX -= 410
...
def compareCounts(surface):
    ...
    if dealerBust and allPlayersBusted:
        ...
        for player in players:
            if player.bust:
                add_text(f"{player.name} busted. YOU LOSE.", text_Normal, surface, player.x, player.y - 110, white)
                player.updateScore('loss')
            else:
                add_text(f"{player.name}, YOU WON! The dealer busted.", text_Normal, surface, player.x, player.y - 110, white)
        ...
        for player in players:
            if player.bust:
                ...
                add_text(f"{player.name}, YOU WON!", text_Normal, surface, player.x, player.y - 110, white)
                player.updateScore('win')
            elif player.count == dealer.count:
                add_text(f"{player.name}, YOU TIED.", text_Normal, surface, player.x, player.y - 110, white)
                player.updateScore('draw')
            else:
                add_text(f"The Dealer won. {player.name}, YOU LOSE.", text_Normal, surface, player.x, player.y - 110, white)
                player.updateScore('loss')
            startX -= 300
...
def showEndRoundScreen(surface):
    ...
    for i in range(numPlayers - 1, -1, -1):
        ...
        if i == numPlayers - 2 and numPlayers == 2:
            break
    add_text(countString1, text_Normal, surface, halfWidth, startY + 270, orange)

    add_text("Press 'D' to DEAL or Press 'Q' to QUIT", text_Bold, surface, halfWidth, 400, orange)
    pygame.display.update()
    roundEnd = True
    while roundEnd:
        for event in pygame.event.get():
...
def finalRecords():
    ...
    startX = 0
    startY = 0
    add_text("Rounds Completed: " + str(round), text_SubHeading, screen, halfWidth, 250, white)
    for player in players:
      add_text(player.name + " Game Stats:", text_Bold, screen, startX + 180, startY + 350, black)
      add_text(player.displayScore(), text_Score, screen, startX + 180, startY + 400, white)
      startX += 430
...
gameOver = False
ai_minimax = AI_Minimax(Card)
ai_second = AI_Second(Card)
