...
class Player:
    ...
    def aiChoice(self, dealer_hand, remaining_cards):
        """
        Use the AI agent to decide the next action.
        :param dealer_hand: The dealer's visible cards.
        :param remaining_cards: The list of remaining cards in the deck.
        :return: 1 for 'Hit', 2 for 'Stand'
        """
        action = ai_agent.decide_action(self.hand, dealer_hand, remaining_cards)
        if action == "hit":
            return 1  # Hit
        elif action == "stand":
            return 2  # Stand
...
def playTurns():
    ...
    remaining_cards = dealer.deck.cards  # Pass the list of remaining Card objects
    dealer_visible_card = [dealer.hand[0]]  # Pass the visible dealer card (Card object)
    
    while True:
        # Pass both dealer_visible_card and remaining_cards to aiChoice
        choice = player.aiChoice(dealer_visible_card, remaining_cards)  # Use AI logic
